...
void update_st(vi &segment_tree, int L, int R, int current, int updated_node, int new_value) {
    ...
    if(L == R) {
        segment_tree[current] = new_value;
        return;
    }
    int m = (L+R)/2;
...
void hld(vvi &G, int current, int parent, vi &subtree_size, vi &id, int &count, vi &color, int paint, vi &segment_tree, vi &value) {
    id[current] = ++count;
    color[current] = paint;
    update_st(segment_tree, 1, G.size()-1, 1, id[current], value[current]);
...
int query(int u, int v, vi &parent, vi &color, vi &depth, vi &segment_tree, vi &id) {
    int ans = 0;
...
int main() {
    ...
    for(int i=0; i<q; i++) {
        ...
        if(a == 1) {
            value[b] = c;
            update_st(segment_tree, 1, n, 1, id[b], c);
        } else {
            cout << query(b, c, parents, color, depth, segment_tree, id) << ' ';
        }
    }
    cout << '\n';
    return 0;
